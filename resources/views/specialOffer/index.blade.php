...
<script type="text/javascript">
    ...
    $(".save_special_offer_btn").click(function () {
        ...
        database.collection('vendors').doc(vendorId).update({'specialDiscount': specialDiscount,specialDiscountEnable:enabledSpecialOffer,
            // SERVICE_60N_STORE_PANEL_VENDOR_SUBSET_WRITE_CONSOLIDATION_PATCH
            'updatedAt': firebase.firestore.FieldValue.serverTimestamp()}).then(function (result) {
    ...
    function deleteOffer(day, count, i) {
        ...
        database.collection('vendors').doc(vendorId).update({'specialDiscount': specialDiscount,
            // SERVICE_60N_STORE_PANEL_VENDOR_SUBSET_WRITE_CONSOLIDATION_PATCH
            'updatedAt': firebase.firestore.FieldValue.serverTimestamp()}).then(function (result) {
    ...
    async function getVendorId(vendorUser) {
        ...
        await database.collection('vendors').where('author', "==", vendorUser).get().then(async function (vendorSnapshots) {
            ...
            console.log(vendorData.section_id || vendorData.sectionId);
            // SERVICE_60G_STORE_PANEL_SECTION_FIELD_NORMALIZER_PATCH
            var sectionId = vendorData.section_id || vendorData.sectionId || '';

            // get section document from "sections" collection
